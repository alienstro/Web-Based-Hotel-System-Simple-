...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Check if user submitting form using post method
    ...
    try {
        ...
        if ($signupContr->is_input_empty($email, $pwd, $first_name, $last_name)) { // Checks if input empty
            $errors["empty_input"] = "Fill in all fields!";
        } else if ($signupContr->is_email_invalid($email)) { // Checks if email is invalid
            $errors["invalid_email"] = "Invalid email!";
        } else if ($signupContr->is_email_registered($pdo, $email)) { // Checks if there is duplicates
        ...
        $signupContr->create_user($pdo, $role, $email, $pwd, $first_name, $last_name); // Sends data to db
